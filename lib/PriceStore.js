...
const { SLOT_MS, startOfLocalDay, addLocalDays, windowAround, DEFAULT_TZ } = require('./time');
...
class PriceStore {
  ...
  cheapestBlock(from, to, durationSlots) {
    ...
    for (let i = 0; i + durationSlots <= list.length; i++) {
      ...
      if (!best || avg < best.avg - 1e-12) {
        best = { start: block[0].start, end: new Date(block[durationSlots - 1].start.getTime() + SLOT_MS), avg, slots: block };
      }
    }
    return best;
  }
   ...
   */
  level(now, { cheap = 0.9, veryCheap = 0.75, expensive = 1.1, veryExpensive = 1.25 } = {}) {
