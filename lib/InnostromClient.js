...
class InnostromClient {
   ...
   */
  constructor({ meteringCode, token, environment = 'production', baseUrl, fetch: fetchImpl, timeoutMs = 20000, tz = 'Europe/Zurich' }) {
  ...
  async _request(url) {
    ...
    try { body = text ? JSON.parse(text) : null; } catch { /* kein JSON */ }
...
module.exports = {
  InnostromClient, InnostromError, ENVIRONMENTS, TEST_CREDENTIALS, TARIFF_TYPES, COMPONENTS,
  normalizeSlot, workPrice,
};
